...
def load_data(data_dir):
    ...
    for category in range(NUM_CATEGORIES):
        for file in os.listdir(os.path.join(data_dir, str(category))):
            cv2_image = cv2.imread(os.path.join(data_dir, str(category), file))
...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        ...
        # Add a hidden layer with dropout, using 128 units due to experience
        tf.keras.layers.Dense(128, activation ="relu"),
        tf.keras.layers.Dropout(0.05),

        # Add a output layer with output units for all categories
